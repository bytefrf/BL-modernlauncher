...
public static class MultiplayerServerListService
{
    ...
    // seedEmbeddedTemplate=false — НЕ подсевать встроенный шаблон (с tfgm-серверами) при создании
    // нового servers.dat. Нужно для пер-сборочных серверов: у каждой сборки только свой сервер,
    // а не общий tfgm-набор. Уже добавленные игроком сервера в существующем файле сохраняются.
    public static void EnsureServer(string installRoot, string serverName, string serverIp, bool seedEmbeddedTemplate = true)
    {
        ...
        if (File.Exists(serversPath))
        {
            ...
            {
                root = (seedEmbeddedTemplate ? TryReadEmbeddedTemplate() : null) ?? new ServerListNbt();
            }
        }
        else
        {
