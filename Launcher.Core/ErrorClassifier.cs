...
public static class ErrorClassifier
{
    ...
    {
        // ДО всех сетевых веток: отказ по хосту Mojang — не «проблема с интернетом» и тем более не
        // «сайт лаунчера недоступен». Ветка ниже про «installer could not reach» ловила только отказ
        // Forge-инсталлера, а когда ванильные файлы качает сам лаунчер (client.jar, libraries, assets),
        // тот же отказ классифицировался как обычная ошибка сети и игрок чинил не то.
        if (IsMojangHostFailure(exception))
        {
            return Create(
                "Нет доступа к серверам Mojang",
                "Лаунчер качает файлы Minecraft напрямую с серверов Mojang, а с этого компьютера они не открываются. " +
                "Обычно это блокировка провайдера, DNS или фильтр антивируса. Папка установки и её путь тут ни при чём.",
                [
                    "Смени DNS на 1.1.1.1 или 8.8.8.8 либо попробуй другую сеть (например, раздачу с телефона).",
                    "Если пользуешься VPN — включи его и запусти установку ещё раз.",
                    "Уже скачанное не потеряется: установка продолжится с того же места."
                ],
                exception);
        }
        ...
        // Сервер сборки не ответил и рабочего зеркала в манифесте нет — не сетевой сбой у игрока.
        if (message.Contains(Services.ArchiveInstallService.NoArchiveSourceMessage, StringComparison.Ordinal))
        {
            return Create(
                "Сервер сборки недоступен",
                "Лаунчер не смог получить адрес архива сборки: сайт не ответил, а запасного адреса в манифесте нет.",
                [
                    "Подожди и запусти лаунчер ещё раз — обычно это временно.",
                    "Проверь, открывается ли https://bl-modern.ru в браузере.",
                    "Установленная сборка не пострадала, перекачивать ничего не нужно."
                ],
                exception);
        }

        // Установщик Forge/NeoForge не достучался до серверов Mojang. Проверяем ДО общих проверок про
        // Java и Forge: в тексте есть и путь к java.exe, и «installer failed», из-за чего такой отказ
        // раньше классифицировался как «Проблема с Java» и игрок чинил не то.
        if (message.Contains("installer could not reach", StringComparison.OrdinalIgnoreCase))
        {
            return Create(
                "Нет доступа к серверам Mojang",
                "Установщик Forge докачивает часть файлов напрямую с серверов Mojang, а с этого компьютера они не открываются. Обычно это блокировка провайдера, DNS или фильтр антивируса.",
                [
                    "Попробуй другую сеть (например, раздачу с телефона) или смени DNS на 1.1.1.1 / 8.8.8.8.",
                    "Если пользуешься VPN — включи его и запусти установку ещё раз.",
                    "Уже скачанное не потеряется: установка продолжится с того же места."
                ],
                exception);
    ...
    /// <summary>
    /// Сетевой отказ именно на серверах раздачи Minecraft. Имя хоста берём из текста исключения:
    /// лаунчер оборачивает загрузку в сообщение с полным URL («Не удалось загрузить файл … https://…»),
    /// поэтому хост доезжает до классификатора даже через несколько уровней вложенности.
    /// </summary>
    private static bool IsMojangHostFailure(Exception exception)
    {
        var message = exception.ToString();
        if (!MojangDownloadHosts.Any(host => message.Contains(host, StringComparison.OrdinalIgnoreCase)))
        {
            return false;
        }
        ...
        {
            // DNS не резолвится (блокировка/подмена) или соединение рвут на подключении/TLS —
            // с точки зрения игрока это одна и та же беда: до Mojang не достучаться.
    ...
    private static bool IsNetworkIoError(IOException exception)
    {
        ...
        // Сбой расшифровки TLS (SSPI 0x80090330) прилетает как IOException без SocketException внутри.
        // Так выглядит HTTPS-фильтр антивируса, рвущий загрузку, — это сеть, а не занятый файл на диске.
        for (Exception? current = exception; current is not null; current = current.InnerException)
        {
            if (current is System.Security.Authentication.AuthenticationException
                || current.HResult == unchecked((int)0x80090330))
            {
                return true;
            }
        }
        ...
        return text.Contains("transport connection", StringComparison.OrdinalIgnoreCase)
            ...
            || text.Contains("decryption operation failed", StringComparison.OrdinalIgnoreCase)
            || text.Contains("не могут быть расшифрованы", StringComparison.OrdinalIgnoreCase)
