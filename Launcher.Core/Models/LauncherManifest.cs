...
{
    // Id сборки из каталога — нужен для per-pack изоляции папки установки при кастомном пути.
    ...
    // Мажорная версия Java, которую требует сборка (17 для Forge 1.20.1, 21 для NeoForge 1.21.1).
    // Используется при выборе управляемой Java (.launcher/runtime/java-{major}) и при её валидации.
